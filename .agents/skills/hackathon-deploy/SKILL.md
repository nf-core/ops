...
- [prerequisites.md](references/prerequisites.md) - First-time setup, secrets, SSH keys
- [oauth-setup.md](references/oauth-setup.md) - GitHub OAuth configuration
- [service-reference.md](references/service-reference.md) - Per-service technical details
...
Should return zone ID. If missing, create zone and configure Netlify NS delegation (see [prerequisites.md](references/prerequisites.md)).
...
Should list keys. If empty, configure 1Password SSH agent (see [prerequisites.md](references/prerequisites.md)).
...
```bash
hackathon-infra/scripts/bootstrap.sh
```
...
For credential generation, see [service-reference.md](references/service-reference.md#testing-turn).
...
```bash
hackathon-infra/scripts/ssh.sh wa    # or lk, turn, jitsi
cloud-init status
docker ps
...
The `hackathon-infra/scripts/ssh.sh` script opens an interactive SSH session but **does not accept commands as arguments**. To execute commands:

```bash
# Get the IP first
hackathon-infra/scripts/ssh.sh wa  # Note the IP shown
