...
def _create_s3_policy_document(bucket_arn: str) -> Dict[str, Any]:
    """Create S3 bucket access policy document with multipart upload support.

    Includes permissions for:
    - Basic bucket operations (list, get location)
    - Object operations (get, put, tag, delete)
    - Multipart upload operations (for large files >5GB)

    Args:
        bucket_arn: ARN of the S3 bucket to grant access to

    Returns:
        Dict[str, Any]: S3 policy document with enhanced permissions
    """
    return {
        "Version": "2012-10-17",
        "Statement": [
    ...
    combined_policies = json.dumps(
        {
            ...
            # Version updated to include multipart upload permissions
            "s3": {"bucket_dependent": True, "version": "v2-multipart"},
        },
        sort_keys=True,
    )
